...
import { assertSafeUrl, BlockedUrlError } from "@accesslint/cli/ssrf-guard";
...
describe("safeFetch", () => {
  ...
  function mockFetchSequence(
    responses: Array<{ status?: number; headers?: Record<string, string>; body?: string | null }>,
  ) {
    let idx = 0;
    vi.stubGlobal(
      "fetch",
  ...
  it("re-validates redirect targets against the SSRF guard", async () => {
    mockFetchSequence([{ status: 302, headers: { location: "http://169.254.169.254/" } }]);
  ...
  it("re-validates redirect targets against private-network policy", async () => {
    mockFetchSequence([{ status: 302, headers: { location: "http://127.0.0.1:8080/admin" } }]);
  ...
  it("caps redirect depth", async () => {
    ...
    await expect(safeFetch("https://example.com/1", { maxRedirects: 3 })).rejects.toThrow(
      /Too many redirects/,
    );
  });

  it("rejects oversized responses via Content-Length", async () => {
    mockFetchSequence([{ status: 200, headers: { "content-length": "99999999" }, body: "x" }]);
