...
export const objectAlt: Rule = {
  ...
  browserHint:
    "Screenshot the embedded object to see its content, then add aria-label or title describing it.",
  ...
  run(doc) {
    ...
    for (const obj of doc.querySelectorAll("object")) {
      ...
      const isImage =
        type.startsWith("image/") || /\.(png|jpg|jpeg|gif|svg|webp|bmp|ico)$/i.test(data);
      if (!isImage) {
