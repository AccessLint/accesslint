...
Violation identity is `ruleId + getSelector(v.element)`. Because Jest runs in jsdom / happy-dom with no browser, the selector is a tag-path rather than an ARIA-role selector — baselines are more sensitive to DOM refactors than the browser-based Playwright equivalent.
